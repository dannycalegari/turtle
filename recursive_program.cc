...
void turtle::execute_recursive_program(recursive_program R, int type, int depth){
	int k;
	cmat save_state;
	long save_pen_color;
	...
	if(depth>=0){
		for(k=0;k<(int) R.options[type].size();k++){
			...
			execute_elementary_program(R.subprogram[R.options[type][k]]);
			execute_recursive_program(R,R.transition[type][k],depth-1);
